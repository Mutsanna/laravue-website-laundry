<template>
    <div class="col-md-12" v-if="$can('read couriers')">
        <div class="panel">
                ...
                <b-table striped hover bordered :items="couriers.data" :fields="fields" show-empty>
                  ...
                  <!-- KITA MENGGUNAKAN CUSTOM TEMPLATE KARENA AKAN MENAMPILKAN GAMBAR -->
                    <template v-slot:cell(photo)="row">
                        <img :src="'/storage/couriers/' + row.item.photo" :width="100" :height="100" :alt="row.item.name">
                    </template>
                    <template v-slot:cell(outlet_id)="row">
                        {{ row.item.outlet.name }}
                    </template>
                    <template v-slot:cell(actions)="row">
                        <router-link :to="{ name: 'couriers.edit', params: {id: row.item.id} }" class="btn btn-warning btn-sm" v-if="$can('edit couriers')"><i class="fa fa-pencil"></i></router-link>
                        <button class="btn btn-danger btn-sm" @click="deleteCourier(row.item.id)" v-if="$can('delete couriers')"><i class="fa fa-trash"></i></button>
                    </template>
                </b-table>
