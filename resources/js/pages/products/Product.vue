<template>
    <div class="col-md-12" v-if="$can('read products')">
        <div class="panel">
            <div class="panel-heading">
                <router-link :to="{ name: 'products.add' }" class="btn btn-primary btn-sm btn-flat" v-if="$can('create products')">Tambah</router-link>
            ...
            <div class="panel-body">
                <b-table striped hover bordered :items="products.data" :fields="fields" show-empty>
                    ...
                    <template v-slot:cell(actions)="row">
                        <router-link :to="{ name: 'products.edit', params: {id: row.item.id} }" class="btn btn-warning btn-sm" v-if="$can('edit products')"><i class="fa fa-pencil"></i></router-link>
                        <button class="btn btn-danger btn-sm" @click="deleteProduct(row.item.id)" v-if="$can('delete products')"><i class="fa fa-trash"></i></button>
                    </template>
                </b-table>
